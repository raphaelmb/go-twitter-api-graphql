...
func TestTokenService_CreateAccessToken(t *testing.T) {
	t.Run("can create a valid access token", func(t *testing.T) {
		...
		tok, err := jwtGo.Parse([]byte(token), jwtGo.WithValidate(true), jwtGo.WithVerify(signatureType, []byte(conf.JWT.Secret)), jwtGo.WithIssuer(conf.JWT.Issuer))
		require.NoError(t, err)
...
func TestTokenService_ParseToken(t *testing.T) {
	...
	t.Run("return err if access token is expired", func(t *testing.T) {
		ctx := context.Background()
		user := twitter.User{
			ID: "123",
		}
...
func TestTokenService_ParseTokenFromRequest(t *testing.T) {
	t.Run("can parse an access token from request", func(t *testing.T) {
		ctx := context.Background()
		user := twitter.User{
			ID: "123",
		}
	...
	t.Run("invalid token will fail", func(t *testing.T) {
		ctx := context.Background()
